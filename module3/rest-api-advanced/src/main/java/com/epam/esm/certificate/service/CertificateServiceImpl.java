...
public class CertificateServiceImpl implements CertificateService {
    ...
    @Override
    public PagedModel<GiftCertificateDto> findAll(String sortField, String sortDirection, int offset, int size) {
        PagedModel<GiftCertificateModel> models = certificateRepository
                .findAll(sortField, sortDirection, offset, size);
    ...
    @Override
    public PagedModel<GiftCertificateDto> find(String key, List<String> tagNames, String sortField, String sortDirection, int offset, int size) {
        if (key.isEmpty() && tagNames.isEmpty()) {
            return findAll(sortField, sortDirection, offset, size);
        } else if (tagNames.isEmpty()) {
            PagedModel<GiftCertificateModel> models = certificateRepository
                    .findByName(key, sortField, sortDirection, offset, size);
            return CertificateDtoMapper.INSTANCE.toDto(models);
        }
        PagedModel<GiftCertificateModel> models = certificateRepository
                .findByTagAndName(key, tagNames, sortField, sortDirection, offset, size);
        return CertificateDtoMapper.INSTANCE.toDto(models);
    }
    ...
    public GiftCertificateDto save(GiftCertificateDto certificateDto) {
        ...
        GiftCertificateModel certificate = CertificateDtoMapper.INSTANCE.fromDto(certificateDto);
